<template>
    <v-card-text>
        ...
        <v-form ref="form" v-model="valid">
            <v-row class="mt-3" id="form">
                <v-col class="col-md-6 col-12">
                    <v-text-field label="userName" prefix="" outlined :rules="[
                    $rules.required,
                    $rules.name]" v-model="user.userName"></v-text-field>
                </v-col>
                <v-col class="col-md-6 col-12">
                    <v-text-field label="email" prefix="" type="email" :rules="[
                    $rules.required,
                    $rules.email]" outlined v-model="user.email">
                    </v-text-field>
                </v-col>
                <v-col class="col-md-6 col-12">
                    <v-text-field v-model="user.password" :append-icon="showPassword ? 'mdi-eye' : 'mdi-eye-off'"
                        :rules="[$rules.required, $rules.password]" :type="showPassword ? 'text' : 'password'"
                        :label="$t('Password')" @click:append="showPassword = !showPassword" outlined dense>
                    </v-text-field>
                </v-col>
                <v-col class="col-md-6 col-12">
                    <v-text-field v-model="user.passwordConfirm" :append-icon="showPassword ? 'mdi-eye' : 'mdi-eye-off'"
                        :type="showPassword ? 'text' : 'password'" :label="$t('Re-enter The Password')"
                ...
                <v-col class="col-md-6 col-12">
                    <v-select :items="types" :rules="[
                    $rules.required,
                    $rules.select]" v-model="user.type" label="type"></v-select>
                </v-col>

                <v-col class="col-md-6 col-12">
                    <v-select :items="countries" :rules="[
                    $rules.required,
                    $rules.select]" v-model="user.location" label="location"></v-select>
                </v-col>
                <v-col class="col-md-6 col-12">
                    <v-text-field label="phone" :rules="[
                    $rules.required,
                    $rules.number]" type="text" v-model="user.phone" outlined></v-text-field>
                </v-col>

                <v-col class="col-md-6 col-12">
                    <v-text-field label="details" v-model="user.details" outlined></v-text-field>
                </v-col>
                <div class="col-12 text-center">
                    <v-btn depressed :disabled="!valid" color="primary" @click="AddUser">
                        save
                    </v-btn>
                </div>
            </v-row>
        ...
        <v-snackbar v-model="snackbar" absolute right color="#f68c28" rounded="pill" centered>
