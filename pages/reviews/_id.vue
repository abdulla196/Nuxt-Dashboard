        ...
        <v-form ref="form" v-model="valid">
                    <v-row id="form">
                        <v-col cols="12" class="my-3">
                            <v-select v-model="ReviewEdit.user_id" :items="allUsersList.data" item-text="userName"
                                item-value="_id" :rules="[$rules.required, $rules.select]" label="Select user" attach
                                chips></v-select>

                        </v-col>
                        <v-col cols="12" class="my-3">
                            <v-select v-model="ReviewEdit.maid_id" :items="allMaidsList.data" item-text="userName"
                                item-value="_id" :rules="[$rules.required, $rules.select]" label="Select maids" attach
                                chips></v-select>

                        </v-col>
                        <v-col cols="12">
                            <v-textarea outlined label="comment" v-model="ReviewEdit.comment"
                                :value="ReviewEdit.comment">
                            </v-textarea>
                        </v-col>
                        <v-col cols="12">
                            <v-rating v-model="ReviewEdit.rate" color="gold" :rules="[ $rules.required]"
                                icon-label="custom icon label text {0} of {1}"></v-rating>
                        </v-col>
                        <div class="col-12 text-center">
                            <v-btn depressed :disabled="!valid" color="primary" @click="UpdateReview">
                                save
                            </v-btn>
                        </div>
                    </v-row>
