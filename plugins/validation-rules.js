...
export default ({ app }) => {
    ...
    Vue.prototype.$rules = {
        ...
        name: (v) => (v && v.length <= 15) || 'Name must be less than 8 characters',
        confirmPassword: (v, password) =>
            v === password || app.i18n.t('The password confirmation does not match.'),
        email: (v) => /.+@.+\..+/.test(v) || app.i18n.t('Email must be valid'),
        number: (v) => {
            let p = /^[0-9]+$/g
            return p.test(v) || app.i18n.t('This field must be numbers only.')
        },
        select: (v) => !!v || app.i18n.t('File is required'),
        fileRequired: (v) => !!v || app.i18n.t('File is required'),
        filesRequired: (v) => (v && v.length > 0) || app.i18n.t('File is required'),
        date: (v) =>
            new Date(v) >= new Date(Date.now() - 24 * 60 * 60 * 60) || console.log(v), // app.i18n.t("Date must be valid"),
    }
}